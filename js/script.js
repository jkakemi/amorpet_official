document.addEventListener("DOMContentLoaded", function () {
  ...
  function atualizarBadge() {
    ...
    if (animalSelecionado === "cachorro") {
      //const dogDisponivel = Math.floor(Math.random() * 100);
      const dogDisponivel = gerarNum();
      badgeText = `Disponíveis para Adoção (Cachorro): ${dogDisponivel}`;
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  function validarFormulario() {
    ...
    if (formValido) {
      //formContato.submit(); Aqui ele consegue pegar os dados
      mostrarDados();
    }
  }
